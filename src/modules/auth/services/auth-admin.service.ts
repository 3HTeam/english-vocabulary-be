...
export class AuthAdminService extends AuthBaseService {
  ...
  async verifyEmail(verifyEmailDto: VerifyEmailDto) {
    const result = await this.verifyEmailInternal(verifyEmailDto);
    return this.formatResponse(result);
  }

  /**
   * Xác thực email nội bộ
   */
  private async verifyEmailInternal(verifyEmailDto: VerifyEmailDto) {
    const { email, otp } = verifyEmailDto;
    if (!email) {
      throw new Error('Vui lòng cung cấp email để xác thực');
    }
    if (!otp) {
      throw new Error('Vui lòng cung cấp OTP để xác thực');
    }
    ...
    if (!user.emailVerificationOtp || !user.emailVerificationExpires) {
      throw new Error('OTP không tồn tại, vui lòng đăng ký lại');
    }
